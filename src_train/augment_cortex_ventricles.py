...
if not os.path.exists(labfname):
    ...
    if 1:
        ...
        wseedmap2 = np.where(hippo_latvent_map)

        hippo_vent_map = ventimg.get_data() == 1
        wseedmap1 = np.where(hippo_vent_map)
...
for numid in range(7, 10): # TWEAK: how many image to generate ? (also affects output filename)
    t1= time.time()
    ...
    kernel2_s = [x[::1,::1,::1][2:-2,2:-2,2:-2] for x in (kernel0_0, kernel0_1, kernel0_2)]
    ...
    if 1: # TWEAK: do?
        ...
        for p in ws:
            scales_xyz = np.random.normal(loc=1, scale=1, size=3) # >0 expansion; <0 shrinking
        ...
        mul_fact = 80.
        kern = (28, 28, 28) # lower more distorted (5..25)
    ...
    if 1: # TWEAK: do?
        ws = np.random.randint(0, len(wseedmap2[0]), size=6) # TWEAK: size=How many small deformation blob
        for p in ws:
            scales_xyz = np.random.normal(loc=1, scale=1, size=3) # >0 expansion; <0 shrinking
            for k, s, c in zip(kernel2_s, scales_xyz, [canvasx, canvasy, canvasz]):
        ...
        # TWEAK: magnitude of warp effect (need to be adjusted by trial and error)
        mul_fact = 8.
        kern = 12 # the lower, the more distorted
    ...
    if 1: # TWEAK
        ...
        angles = np.random.normal(scale=pi/32., size=3) * 0#0.25 # (.05, .05, .05) # TWEAK: amount of rotation motion on x,y,z
        rotation_matrix = dot(  dot( dot(Rx(angles[0]), diag(zooms)), Ry(angles[1]) ), Rz(angles[2])  )

        center_of_rotation_mm = dot(affineh, [x/4. for x in sh] + [1])[:-1] # pick center of the box for rotation center
        center_of_rotation_mm += np.random.normal(scale=1, size=3) * (1., 1., 1.)

        # Translation too
        rotation_matrix[3] = np.random.normal(scale=5, size=4) # units is mm, careful of your input space
    ...
    # The ANTS command
    # warp the main image:
    os.system ("antsApplyTransforms -d 3 -i %s.nii.gz -r %s.nii.gz  -o generated/%s.nii.gz -t %s_replop0Affine.txt  -t %s_replop1Warp.nii --float %s > /dev/null" % (subjid, subjid, prefix, prefix, prefix, fake_background))
    ...
    if 1: # TWEAK: warp other similar images, such as labels ?
        ...
        os.system ("antsApplyTransforms -d 3 -i %s.nii.gz -r %s.nii.gz  -o generated/%s_labels.nii.gz -t %s_replop0Affine.txt -t %s_replop1Warp.nii --float -n MultiLabel[0.001] > /dev/null" % (subjid.replace("orig", "hippoAmyg"), subjid, prefix, prefix, prefix))
        ...
        os.system ("antsApplyTransforms -d 3 -i %s.nii.gz -r %s.nii.gz  -o generated/%s_ventricles.nii.gz -t %s_replop0Affine.txt -t %s_replop1Warp.nii  --float -n NearestNeighbor > /dev/null" % (subjid.replace("orig", "ventricles"), subjid, prefix, prefix, prefix))
